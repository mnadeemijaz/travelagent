...
<style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .total-row td { background: #f3f4f6; font-weight: bold; font-size: 11px; }
    .grand-total td { background: #e5e7eb; font-weight: bold; font-size: 13px; }
</style>
</head>
<body>
...
<table style="margin-bottom:2px">
    <tr>
        ...
        <td style="background:#e5e7eb;font-weight:bold;font-size:11px;border:none;text-align:right">Date: {{ $voucher['date'] }}</td>
    </tr>
    <tr>
...
{{-- Financial Summary --}}
@php
    $sr              = $voucher['sr_rate'] ?? 1;
    $adultAmt        = ($voucher['adult_rate']  ?? 0) * $sr * $voucher['t_adult'];
    $childAmt        = ($voucher['child_rate']  ?? 0) * $sr * $voucher['t_child'];
    $infantAmt       = ($voucher['infant_rate'] ?? 0) * $sr * $voucher['t_infant'];
    $visaTotal       = $adultAmt + $childAmt + $infantAmt;
    $accomTotal      = collect($hotels)->sum(fn($h) => ($h['price'] ?? 0) * $sr * ($h['city_nights'] ?? 0));
    $ziaratTotal     = collect($ziarats)->sum('amount') * $sr;
    $grandTotal      = $visaTotal + $accomTotal + $ziaratTotal;
@endphp

<div class="section-heading">Accounts Detail</div>

{{-- Accommodation breakdown --}}
@if(count($hotels) > 0)
<table style="margin-top:4px">
    <tr>
        <th>Hotel</th><th>City</th><th>Room Type</th><th>Nights</th><th>Rate/Night</th><th>Amount</th>
    </tr>
    @foreach($hotels as $h)
    <tr>
        <td>{{ $h['hotel_name'] ?? '—' }}</td>
        ...
        <td>{{ number_format(($h['price'] ?? 0) * $sr) }}</td>
        <td>{{ number_format(($h['price'] ?? 0) * $sr * ($h['city_nights'] ?? 0)) }}</td>
    </tr>
    @endforeach
    <tr class="total-row">
        <td colspan="5" style="text-align:right"><strong>Accommodation Total</strong></td>
        <td><strong>{{ number_format($accomTotal) }}</strong></td>
    </tr>
</table>
@endif

{{-- Visa summary --}}
@if($voucher['adult_rate'] || $voucher['child_rate'] || $voucher['infant_rate'])
<table style="margin-top:6px">
    ...
    <tr>
        ...
        <td>{{ number_format(($voucher['adult_rate'] ?? 0) * $sr) }}</td>
        <td>{{ number_format(($voucher['child_rate'] ?? 0) * $sr) }}</td>
        <td>{{ number_format(($voucher['infant_rate'] ?? 0) * $sr) }}</td>
    </tr>
    <tr class="total-row">
        <td colspan="3" style="text-align:right"><strong>Visa Amount</strong></td>
        <td><strong>{{ number_format($visaTotal) }}</strong></td>
    </tr>
</table>
@endif
...
<table style="margin-top:6px">
    <tr>
        <td><strong>Ziarat</strong></td>
        <td>{{ collect($ziarats)->pluck('name')->implode(', ') }}</td>
        <td style="text-align:right"><strong>Ziarat Total</strong></td>
        <td><strong>{{ number_format($ziaratTotal) }}</strong></td>
    </tr>
</table>
@endif

...
interface Booking {
    id: number; passengers: number; contact_phone: string | null;
...
export default function GroupTicketBookings({
    bookings, categories, filters, flash,
}: {
    bookings: Paginated<Booking>; categories: string[];
    filters: { status?: string; category?: string }; flash?: { success?: string };
}) {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex flex-col gap-4 p-6">
                <h1 className="text-2xl font-semibold">Group Ticket Bookings</h1>
                ...
                <div className="flex gap-3 flex-wrap">
                    ...
                    <select value={filters.category ?? ''} onChange={e => filter('category', e.target.value)}
                    ...
                    <table className="w-full text-sm">
                        <thead className="bg-muted/50 text-muted-foreground">
                            <tr>
                                ...
                                <th className="px-3 py-3 text-left font-medium">Booked By</th>
                        ...
                        <tbody className="divide-y">
                            {bookings.data.length === 0 && (
                                <tr><td colSpan={10} className="px-4 py-8 text-center text-muted-foreground">No bookings found.</td></tr>
                            )}
                            {bookings.data.map((b, i) => (
                                <tr key={b.id} className="hover:bg-muted/30">
                                    ...
                                    <td className="px-3 py-2">
                                        ...
                                        <div className="text-xs text-muted-foreground capitalize">{b.ticket.airline} · {b.ticket.category}</div>
                                    </td>
                                    <td className="px-3 py-2">{b.ticket.dep_date}</td>
                                        ...
                                        {b.status === 'pending' && (
                                            <div className="flex items-center justify-end gap-2">
                                                <Button size="sm" className="bg-green-600 hover:bg-green-700 h-7 px-3 text-xs"
                                                    onClick={() => approve(b.id)}>Approve</Button>
                                                <Button size="sm" variant="destructive" className="h-7 px-3 text-xs"
                                                    onClick={() => reject(b.id)}>Reject</Button>
                                            </div>
                                        )}
                                        {b.status !== 'pending' && <span className="text-xs text-muted-foreground">—</span>}
                                    </td>
                                </tr>
                            ))}
                        </tbody>

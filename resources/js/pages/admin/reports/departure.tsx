...
interface VoucherRow {
    id: number; date: string | null; dep_time: string | null;
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Reports', href: '#' },
    { title: 'Departure Report', href: '/admin/reports/departure' },
];

export default function DepartureReport({ records, agents, filters }: { records: Paginated<VoucherRow>; agents: Agent[]; filters: Filters }) {
    const { data, setData, get, processing } = useForm({
        start_date: filters.start_date ?? '',
        end_date: filters.end_date ?? '',
        agent_id: filters.agent_id ?? '',
    });
    function search(e: React.FormEvent) { e.preventDefault(); get('/admin/reports/departure', { preserveState: true, replace: true }); }

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Departure Report" />
            <div className="flex flex-col gap-4 p-6">
                <h1 className="text-2xl font-semibold">Departure Report</h1>
                    ...
                    <table className="w-full text-xs">
                        <thead className="bg-muted/50 text-muted-foreground">
                            <tr>
                                ...
                                <th className="px-3 py-2 text-left">Dep Date</th>
